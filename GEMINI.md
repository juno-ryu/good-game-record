...
- `README.md` 파일에는 프로젝트의 전반적인 니즈에 대한 PRD 문서가 정의 되어있습니다. 가장 처음 해야할 일은 PRD 문서를 파악하여 사용자가 원하는 니즈를 파악하는 것입니다.
- `PRD/PRD1~*.md` 파일은 `README.md`파일을 작은단위로 업무분할한 문서로 보면 됩니다. (작은단위로 쪼개어 만들어진 스프린트) 사용자가 지정한 PRD문서를 참고하여 작업을 진행하세요 작업도중 업데이트 사항이 발생한다면 즉시 업데이트 하세요 추가적으로 요구사항이 완료된다면 파일명 뒤에 완료 되었다는 suffix를 추가하여 다음작업시 읽지않도록 하세요
- 파일을 생성할때는 `use-store-hook.tsx` 와 같은 케밥케이스를 사용하세요
- `components`, `hook`, `util`, `store` ...등 관련된 모든 파일들은 `shared` 디렉토리 내부에서 생성하세요
- `radix-ui` 컴포넌트를 적극적으로 사용하고 스타일링은 `tailwind`로 적용하세요
...
2.  `Next.js fetch`: 캐싱 및 재검증을 위해 Next.js 내장 `fetch` 함수 활용
3.  `zustand`: 가벼운 전역 상태 관리
4.  `radix-ui`: 웹접근성을 보장하는 UI컴포넌트
5.  `Icons`: `lucide-react`를 사용하세요.
6.  `yup`: 스키마 유효성 검사 및 데이터 무결성
7.  `react-hook-form`: 폼 유효성 검사 및 상태 관리
